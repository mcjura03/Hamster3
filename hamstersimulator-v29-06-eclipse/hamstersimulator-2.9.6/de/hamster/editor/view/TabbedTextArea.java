...
import java.util.Map; // jrahn: Map-Interface für typisierte Collections ergänzt
...
public class TabbedTextArea extends JPanel implements PropertyChangeListener,
    ...
    protected Map<HamsterFile, TextArea> textAreas; // jrahn: Raw HashMap durch typisierte Map ersetzt

    protected Map<TextArea, JScrollPane> scrollPanes; // jrahn: Raw HashMap durch typisierte Map ersetzt
    ...
    public boolean ensureSaved(HamsterFile file) {
        if (file.isDirectory()) {
            ...
            for (HamsterFile file1 : files) {
                if (!this.ensureSaved(file1)) {
                    return false;
                }
            }
        } else {
    ...
    public TextArea openFile(HamsterFile file) {
        ...
        if (this.textAreas.get(file) != null) {
            TextArea h = this.textAreas.get(file); // jrahn: unnötigen Cast entfernt
            ...
            switch (file.getType()) {
                case HamsterFile.SCRATCHPROGRAM -> {
    ...
    public void closeFile(HamsterFile file) {
        ...
        if (file.isDirectory()) {
            ...
            JScrollPane s = this.scrollPanes.get(h); // jrahn: unnötigen Cast entfernt
    ...
    public void propertyChange(PropertyChangeEvent evt) {
        ...
        TextArea textArea = this.textAreas.get(file); // jrahn: unnötigen Cast entfernt
        int index = this.getIndex(textArea);
    ...
    private TextArea getTextArea(int index) {
        ...
        switch (view) {
            case ScratchPanel scratchPanel -> {
                return scratchPanel.getTextArea();
            }
            case FsmPanel fsmPanel -> {
                return fsmPanel.getTextArea();
            }
            case FlowchartPanel flowchartPanel -> {
                return flowchartPanel.getTextArea();
            }
            default -> {
    ...
    public TextArea getTextArea(HamsterFile file) {
        return this.textAreas.get(file); // jrahn: unnötigen Cast entfernt
    }

    private int getIndex(TextArea textArea) {
        return this.tabbedPane.indexOfComponent(this.scrollPanes.get(textArea)); // jrahn: unnötigen Cast entfernt
    }
